...
async fn main() -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        cleanup::run_cleanup_task(state_clone).await;
    });

    // Moderate rate limiting: 10 burst, 1 request per second refill rate
    ...
    let app = Router::new()
        ...
        .route(
            "/git/:slug/git-upload-pack",
            post(git::smart_http::upload_pack),
        )
